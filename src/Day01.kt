 ...
 */
fun getElfCalories(input: List<String>): HashMap<Int, Int> {
    val elfCalorieMap: HashMap<Int, Int> = HashMap()
    ...
    input.forEach { calorie ->
        if (calorie.isEmpty()) {
            ...
            elfCalorieMap[elfIndex] = (elfCalorieMap[elfIndex] ?: 0) + calorie.toInt()
        }
    }
...
fun main() {
    ...
    val elfCalorieMap = getElfCalories(input)

    // Part 1 result
    println(elfCalorieMap.values.max())

    // Part 2 result
    println(elfCalorieMap.values.sortedDescending().take(3).sum())
}
