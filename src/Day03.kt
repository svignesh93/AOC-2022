 ...
 * All the badges need to be pulled out of the rucksacks so the new authenticity stickers can be attached.
 *
...
fun main() {
    ...
    fun prioritySumOfCompartmentItems(): Int {
        var prioritySum = 0
        input.forEach { rucksack ->
            ...
            for (itemIndex in secondCompartment.indices) {
                val item = secondCompartment[itemIndex]
                if (firstCompartment.contains(item)) {
                    prioritySum += priorityMap[item] ?: 0
                    break
                }
            }
        }
    ...
    fun prioritySumOfGroupItems(): Int {
        var prioritySum = 0
        for (index in input.indices step 3) {
            val rucksackOne = input[index].toSet()
            val rucksackTwo = input[index + 1]
            val rucksackThree = input[index + 2]

            val commonItemSet = HashSet<Char>()

            for (itemIndex in rucksackTwo.indices) {
                val item = rucksackTwo[itemIndex]
                if (rucksackOne.contains(item)) {
                    commonItemSet.add(item)
                }
            }

            for (itemIndex in rucksackThree.indices) {
                val item = rucksackThree[itemIndex]
                if (commonItemSet.contains(item)) {
                    prioritySum += priorityMap[item] ?: 0
                    break
                }
            }
    ...
    // Part 1
    println("Priority Sum Of Compartments: ${prioritySumOfCompartmentItems()}")

    // Part 2
    println("Priority Sum of Groups: ${prioritySumOfGroupItems()}")
}
